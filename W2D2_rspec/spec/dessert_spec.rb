...
describe Dessert do
  ...
  let(:chef) { double("chef", name: "Lauren") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cookie", "huge", "Mom")}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Lauren the Great Baker")
      expect(dessert.serve).to eq("Chef Lauren the Great Baker has made 50 brownies!")
    end
  end
