...
class Play
  ...
  def self.find_by_title(title)
    ...
    play = Play.new(play.first)
    #play is stored in an array???
  end

  def self.find_by_playwright(name)
    # plays = PlaywrightDBConnection.instance.execute(<<-SQL, name)
    # SELECT
    #   plays.title
    # FROM
    #   plays
    # JOIN
    #   playwrights ON play.playwright_id = playwrights.id
    # WHERE
    #   name = ?
    # SQL
    # data.map {|datum| Play.new(datum)}
...
class Playwright
  ...
  def get_plays
    ...
    SQL
    data.map {|datum| Plays.new(datum)}
  end
end
